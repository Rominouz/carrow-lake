...
const AudioCore = {
    ...
    toggle() { 
        this.mute = !this.mute; 
        const icon = $('a-icon'); if(icon) icon.className = this.mute ? 'ti ti-volume-off' : 'ti ti-volume'; 
        if(this.ctx) { this.mute ? this.ctx.suspend() : this.ctx.resume(); } 
    },
    playClick() {
...
function goScreen(id, fade = false, cb = null) {
    ...
    if(fade) {
        ...
        setTimeout(() => {
            ...
            if(cb) cb();
            if(b) setTimeout(() => { b.style.opacity = '0'; }, 100);
        }, 800);
    } else {
        SCREENS.forEach(s => { const el = $(s); if(el) el.classList.remove('active'); });
        const target = $(id); if(target) target.classList.add('active');
...
function startPlay() {
    AudioCore.init();
    if(gs.char) {
        if(gs.scene === 1) { goScreen('ssms'); if(typeof startSMS === 'function') startSMS(true); }
        else if(gs.scene === 2) { goScreen('sn'); if(typeof startNarr === 'function') startNarr(gs.narrIdx); }
        else if(gs.scene === 3) { goScreen('ssms'); if(typeof startScene3 === 'function') startScene3(); }
        else { goScreen('sso'); }
    } else { goScreen('schar'); }
}

function resetGame() { localStorage.removeItem('cl_save'); gs = { char: null, scene: 1, narrIdx: 0 }; updateHi(); goScreen('schar'); }
function pickChar(name) { gs.char = name; gs.scene = 1; gs.narrIdx = 0; sv(); goScreen('ssms'); if(typeof startSMS === 'function') startSMS(false); }

const M = () => $('smsg');
const SC = () => { const m = M(); if(m) m.scrollTop = m.scrollHeight; };

function addIn(t) { AudioCore.playSms(); const d = document.createElement('div'); d.className = 'bi bin'; d.innerHTML = t.replace(/\n/g, '<br>'); if(M()) M().appendChild(d); SC(); }
function addBlk() { AudioCore.playSms(); const d = document.createElement('div'); d.className = 'bbl'; d.innerHTML = '⚠ Cet utilisateur vous a bloqué(e)'; if(M()) M().appendChild(d); SC(); }
function addVu() { const d = document.createElement('div'); d.className = 'bvu'; d.textContent = 'Vu'; if(M()) M().appendChild(d); SC(); }
...
function mkT(id) { const d = document.createElement('div'); d.id = id; d.className = 'bi bin'; d.innerHTML = '...'; if(M()) M().appendChild(d); SC(); }
function rmT(id) { const t = $(id); if(t) t.remove(); }
function clrCh() { const c = $('sch'); if(c) c.innerHTML = ''; }
function addCho(txt, fn) { const b = document.createElement('button'); b.className = 'cho'; b.textContent = txt; b.onclick = () => { AudioCore.playClick(); fn(); }; const c = $('sch'); if(c) c.appendChild(b); }
...
function startNarr(idx, customArr, onComplete){
    ...
    nArr = customArr || (typeof SEGS !== 'undefined' ? SEGS : []);
    nCb = onComplete || null;
    if($('snbg') && BGS && nArr.length > 0) {
        $('snbg').innerHTML=BGS[nArr[nIdx].bg] || BGS[0]; 
        $('snbg').style.opacity='1'; 
        if($('snbg2')) $('snbg2').style.opacity='0';
        nBg=nArr[nIdx].bg;
    }
    updateTx();
}
...
function narrClick(){
    if(!nActive || !nArr || nArr.length === 0) return;
    AudioCore.playClick();
    nIdx++;
    if(nIdx >= nArr.length){
        nActive=false;
        if(nCb) { 
            ...
            goScreen('ssms', true, () => {
                if(typeof startScene3 === 'function') startScene3();
            });
        }
        return;
    }
    if(typeof SEGS !== 'undefined' && nArr === SEGS) { gs.narrIdx=nIdx; sv(); }
    const newBg = nArr[nIdx].bg;
    if(newBg !== nBg && $('snbg') && BGS) { $('snbg').innerHTML = BGS[newBg] || BGS[0]; nBg = newBg; }
    updateTx();
}

function updateTx(){
    if(!nArr || nArr.length === 0 || !$('sntx')) return;
    ...
    if($('snpg-bar')) $('snpg-bar').style.width = Math.round(((nIdx + 1) / nArr.length) * 100) + '%';
}
