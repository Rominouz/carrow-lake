...
const SCREENS = ['sh','schar','ssms','sn','sso'];

const AudioCore = {
    ctx: null, mute: false, windNode: null,
...
function addIn(t) { AudioCore.playSms(); const d = document.createElement('div'); d.className = 'bi bin'; d.textContent = t; M().appendChild(d); SC(); }
...
function addStatusLabel(stId, thenVu, vuDelay) {
    ...
    d.id = stId;
    d.className = 'bst';
    d.style.animation = 'bstIn .4s forwards';
...
function startNarr(idx){
    AudioCore.playWind();
    nIdx=idx; nActive=true; nBg=-1;
    $('snbg').innerHTML=BGS[SEGS[nIdx].bg] || BGS[0]; $('snbg').style.opacity='1'; $('snbg2').style.opacity='0';
    nBg=SEGS[nIdx].bg;
    updateTx();
}
...
function narrClick(){
    ...
    AudioCore.playClick();
    nIdx++;
    if(nIdx >= SEGS.length){ nActive=false; gs.scene=3; gs.narrIdx=SEGS.length; sv(); goScreen('sso'); return; }
    gs.narrIdx=nIdx; sv();
    const newBg = SEGS[nIdx].bg;
    if(newBg !== nBg) { $('snbg').innerHTML = BGS[newBg] || BGS[0]; nBg = newBg; }
    updateTx();
}

function updateTx(){
    const seg = SEGS[nIdx], tx = $('sntx');
    let t = seg.t; 
    ...
    $('snpg-bar').style.width = Math.round(((nIdx + 1) / SEGS.length) * 100) + '%';
}
